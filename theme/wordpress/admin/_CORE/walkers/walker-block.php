...
class poxy_block_menu_walker extends Walker_Nav_Menu {
      ...
      $GLOBALS['dd_children'] = ( isset($children_elements[$element->ID]) )? 1:0;
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
          ...
          if($depth == 1){
            ...
            $item_output = $args->before;
            $item_output .= '<li style="margin-left:0; text-transform:uppercase !important;"><h4 class="epsilon"><a style="color: #03A9F8 !important;" href="'.$item->url.'">';
            //$item_output .= '<a'. $attributes .'>';
            $item_output .= $args->link_before .$prepend.apply_filters( 'the_title', $item->title, $item->ID ).$append;
            $item_output .= $description.$args->link_after;
            $item_output .= '</a></h4>';
    ...
    function start_lvl(&$output, $depth = 0, $args = array()) {
      $indent = str_repeat("\t", $depth);
      if($depth == 0){
        $output .= "\n$indent<div class='sub-menu'><div class='copy-width'>\n$indent<ul class='sub-inside POX_ A_4_14-13 B_4_14-14 G_3_13-13'>\n";
      }
