...
$member_posts = new WP_Query( $args );
 ?><?php if ($member_posts->have_posts()) : ?><?php while ($member_posts->have_posts()) : $member_posts->the_post(); ?><?php global $post; ?><?php $image = wp_get_attachment_image_src(get_post_meta($post->ID, '_poxy_project_fig_1_1_image', true), 'poxy_1000x1500')[0] ? "background-image: url(".wp_get_attachment_image_src(get_post_meta($post->ID, '_poxy_project_fig_1_1_image', true), 'poxy_1000x1500')[0].");" : ''; ?><section class="shaodw-right-inset"><div class="sw"><div class="cw rel"><div class="poxya14poxyb14poxyc11poxyd11"><h2 style="<?php echo $accent_color; ?>" class="fs___5 mr4 mb4">Overview</h2></div><div class="poxya14poxyb14poxyc13poxyd13 tx-2"><div class="title mb2"><h5 style="<?php echo $accent_color; ?>" class="bold mr4 mb4">Studio</h5><?php $terms = get_the_terms( $post->ID , 'project_studio' );
