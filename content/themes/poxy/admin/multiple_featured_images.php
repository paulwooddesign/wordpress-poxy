...
function MultiPostThumbnailsSetAsThumbnail(thumb_id, id, post_type, nonce){
	...
	jQuery.post(ajaxurl, {
		...
		public static function get_the_post_thumbnail_url($post_type, $thumb_id, $post_id = NULL, $size = 'post-thumbnail', $attr = '' , $link_to_original = false) {
			...
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			$img_src = wp_get_attachment_image_src($post_thumbnail_id, "poxy_banner_bg");
			$img_url = $img_src[0];		
			return $img_url;			
		}


		public static function w45_get_the_post_thumbnail_url($post_type, $thumb_id, $post_id = NULL, $size = 'post-thumbnail', $attr = '' , $link_to_original = false) {
			global $id;
			$post_id = (NULL === $post_id) ? $id : $post_id;
			$post_thumbnail_id = self::get_post_thumbnail_id($post_type, $thumb_id, $post_id);
			$img_src = wp_get_attachment_image_src($post_thumbnail_id, "w45_square_thumb_350");
			$img_url = $img_src[0];		
			return $img_url;			
		}
