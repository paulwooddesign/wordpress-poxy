...
class poxy_parallax_menu_walker extends Walker_Nav_Menu {
     function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output ) {
      $GLOBALS['dd_children'] = ( isset($children_elements[$element->ID]) )? 1:0;
        $GLOBALS['dd_depth'] = (int) $depth;
        $id_field = $this->db_fields['id'];
...
function wpse_add_arrow( $item_output, $item, $depth, $args ){
    ...
    if(!is_front_page()) {
      if('main' == $args->theme_location && $depth ==0){
          ...
          if( poxy_has_children($slug) == 0 ) {
          $child_count = poxy_has_children($item->ID);
          $item_output ='<a href="'.$item->url.'" class="three-d">'.$item->title.'<span class="three-d-box"><span class="front">'.$item->title.'</span><span class="back">'.$item->title.'</span></span></a>';
          
          } else {
          $home = get_home_url();
          $item_output ='<a href="'.$home.'/#'.$slug.'" class="three-d">'.$item->title.'<span class="three-d-box"><span class="front">'.$item->title.'</span><span class="back">'.$item->title.'</span></span></a>';
      ...
      if('main' == $args->theme_location && $depth ==0){
          ...
          $item_output ='<a href=".section-'.$slug.'" class="three-d">'.$item->title.'<span class="three-d-box"><span class="front">'.$item->title.'</span><span class="back">'.$item->title.'</span></span></a>';
